...
class FittingOP:
    def __init__(self, fittingconfig):
        ...
        self.z = Variable(torch.zeros(self.batch_size,self.cfg.latentD).to(self.device), requires_grad=True)
        ...
        self.optimizer_s1 = optim.Adam([self.z], lr=self.init_lr_h*2.0)
        self.optimizer_s2 = optim.Adam([self.transl_rec, self.glo_rot_rec], lr=self.init_lr_h*2.0)
        self.optimizer_s3 = optim.Adam([self.betas, self.transl_rec, self.glo_rot_rec, self.vpose_rec], lr=self.init_lr_h*1.5)
        self.optimizer_s4 = optim.Adam([self.vpose_rec, self.hand_pose, self.eye_pose],
                                        lr=self.init_lr_h)
        
        self.optimizers = [self.optimizer_s2,
                           self.optimizer_s1,
                           self.optimizer_s3,
                           self.optimizer_s4]
    ...
    def reset(self):
        self.z.data = torch.nn.Parameter(torch.zeros(self.batch_size,self.cfg.latentD).to(self.device))
    ...
    def calc_loss_contact_map(self, body_markers, verts_object, normal_object, contacts_object, contacts_markers, gender, betas, stage, alpha, extras={}):
        ...
        if alpha_obstacle_in + alpha_obstacle_out > 0:
            loss_obstacle_in, loss_obstacle_out = self.get_obstacle_penet_loss(verts_full_new, extras)
            loss_obstacle_in = torch.mean(loss_obstacle_in) # avgerage over bs
            loss_obstacle_out = torch.mean(loss_obstacle_out) # avgerage over bs
        # print(f"final losss shape: loss_obstacle_in =  {loss_obstacle_in}, loss_obstacle_out = {loss_obstacle_out}")
        
        ############################################################################################################
        loss = (self.cfg.marker_rec_weight * loss_rec
           + (self.only_rec==False) * (self.contact_loss=='contact') * (stage>1) * self.cfg.w_saga_contact * (loss_marker_contact+loss_object_contact)  # 2 / 0 / 0
           + (self.only_rec==False) * (self.contact_loss=='prior') * (stage>1) * 15 * loss_prior_contact   # 2 / 0 / 0
           + (self.only_rec==False) * (stage>1)* self.cfg.w_saga_collision * (h_collision+loss_dist_o)
           + (stage>1)*loss_hand_pose_reg
           + (stage>1)*loss_eye_pose_reg
           + (stage>0)*loss_vpose_reg
           + self.cfg.foot_weight * loss_foot
           + alpha_obstacle_in * loss_obstacle_in # from flex
           + alpha_obstacle_out * loss_obstacle_out # from flex
           )
    ...
    def inference(self, grabpose, z, obj_data):
        ...
        :z: the latent code to be learned
        """
        ### object centered
        ...
        with torch.no_grad():
            verts_object_i = obj_data['verts_object']
            feat_object_i = obj_data['feat_object']
            transf_transl_i = obj_data['transf_transl']
            # print(f"shape of verts_object_i = {verts_object_i.shape}, shape of feat_object_i = {feat_object_i.shape}")
            object_cond_i = grabpose.full_grasp_net.pointnet(l0_xyz=verts_object_i, l0_points=feat_object_i)
            sample_results = grabpose.full_grasp_net.decode(z, object_cond_i, verts_object_i, feat_object_i, transf_transl_i)
            
            markers_gen.append((sample_results[0]+obj_data['transf_transl']))
        ...
        output = {}
        output['markers_gen'] = markers_gen
        output['markers_contact_gen'] = markers_contact_gen
        output['object_contact_gen'] = object_contact_gen
        output['normal_object'] = torch.Tensor(obj_data['normal_object'])#.repeat(n_rand_samples, axis=0)
        output['transf_transl'] = torch.Tensor(obj_data['transf_transl'])#.repeat(n_rand_samples, axis=0)
        output['global_orient_object'] = torch.Tensor(obj_data['global_orient'])#.repeat(n_rand_samples, axis=0)
        output['global_orient_object_rotmat'] = torch.Tensor(obj_data['global_orient_rotmat'])#.repeat(n_rand_samples, axis=0)
        output['verts_object'] = (obj_data['verts_object']+obj_data['transf_transl'].view(-1,3,1).repeat(1,1,2048)).permute(0, 2, 1)#.repeat(n_rand_samples, axis=0)
        output['verts_object'] = torch.Tensor(output['verts_object'])
    ...
    def fitting(self, grabpose, object_info, gender, extras, betas=None):
        if gender == 'male':
        ...
        for ss, optimizer in enumerate(self.optimizers):
            for ii in range(self.num_iter[ss]):
                ...
                # use the decoder to get the marker_based sample results
                samples_results = self.inference(grabpose, self.z, object_info)
                # prepare objects
                verts_object = samples_results['verts_object'].to(self.device)  # (1, 2048, 3)
                normals_object = samples_results['normal_object'].to(self.device) # (1, 2048, 3)
                global_orients_object = samples_results['global_orient_object'].to(self.device) # (1, 2048, 3)
                transf_transl_object = samples_results['transf_transl'].to(self.device) # (1, 2048, 3)

                # prepare body markers
                markers_gen = samples_results['markers_gen'].to(self.device)  # (1, 143, 3)
                object_contacts_gen = samples_results['object_contact_gen'].view(markers_gen.shape[0], -1, 1).to(self.device) #  (1, 2048, 1)
                markers_contacts_gen = samples_results['markers_contact_gen'].view(markers_gen.shape[0], -1, 1).to(self.device)  #  (1, 143, 1)

                loss, loss_dict, markers_fit, body_param, vertices_info, rhand_verts_rec, rh_normals, h2o_signed, o2h_signed = self.calc_loss_contact_map(markers_gen, verts_object, normals_object, object_contacts_gen, markers_contacts_gen, gender, betas, ss, alpha, extras) 
        ...
        return markers_fit_list, smplxparams_list, object_contacts_gen[0], markers_contacts_gen[0], verts_object, global_orients_object[0], transf_transl_object[0]
    # ===========================================================================================================================#
    ...
    def get_obstacle_penet_loss(self, verts_full, extras):
        """
        Compute penetration loss between human and all provided obstacle vertices.

        :param verts_full                   (SMPLX body vertices)
        ...
        bv = verts_full.reshape(bs,-1,3)  # (bs, 10475, 3)
